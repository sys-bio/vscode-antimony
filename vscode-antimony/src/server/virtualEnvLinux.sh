...
ve() {
    ...
    if ((success == 0)); then
        ...
        # ols_client==0.0.9
        AMAS-sb==0.0.1
        orjson==3.8.0" > "$HOME/$venv/requirements.txt"
        python3 -m pip --disable-pip-version-check install -t $HOME/${venv}/lib/python3.10/site-packages --no-cache-dir --upgrade -r $HOME/${venv}/requirements.txt && success=1
        if ((success == 0)); then
            echo "Failed to install Python libs with 'python3'."
        fi
    fi
}
