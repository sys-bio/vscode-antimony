...
ve() {
    ...
    if [[ $cpu_type == *"Apple"* ]]; then
        ...
        while IFS= read -r dependency; do
            ...
            python3 -m pip --disable-pip-version-check install -t $HOME/${venv}/lib/python3.9/site-packages --no-cache-dir --upgrade $dependency
    ...
    while IFS= read -r dependency; do
        echo "Installing $dependency..."
        DYLD_LIBRARY_PATH=/Users/VscodeAntimonySetup/3.1.0/lib /Users/VscodeAntimonySetup/bin/python3.9 -m pip --disable-pip-version-check install -t $HOME/${venv}/lib/python3.9/site-packages --no-cache-dir --upgrade $dependency
        echo "Installed $dependency"
    done < $HOME/${venv}/all-requirements.txt
