...
ve() {
  ...
  sudo -u $USER "$HOME"/VscodeAntimonySetup/bin/python3.9 -m venv $HOME/${venv} --system-site-package
  ...
  while IFS= read -r dependency; do
    ...
    # Execute the command and capture the output
    output=$(DYLD_LIBRARY_PATH="$HOME"/VscodeAntimonySetup/3.1.0/lib $HOME/VscodeAntimonySetup/bin/python3.9 -m pip --disable-pip-version-check install -t $HOME/${venv}/lib/python3.9/site-packages --no-cache-dir --upgrade $dependency 2>&1)
