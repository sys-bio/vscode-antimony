...
async function createVirtualEnv(context: vscode.ExtensionContext) {
	...
	if ((os.platform().toString() == 'darwin') && fs.existsSync(path.normalize(os.homedir() + "/vscode_antimony_virtual_env/bin/python"))) {
		...
		if (os.platform().toString() == 'linux') {
			vscode.window.showInformationMessage('[IMPORTANT: Linux users will have to install python3.10, venv python3.10 package, pip, NodeJS and restart computer before proceeding.] To install dependencies so the extension works properly, allow installation of virtual environment', ...['Yes', 'No'])
			...
			vscode.window.showInformationMessage('[IMPORTANT: Windows users MUST install python3.10 (must be version 3.10), NodeJS and restart computer before proceeding.] To install dependencies so the extension works properly, allow installation of virtual environment', ...['Yes', 'No'])
			.then(async selection => {
				// installing virtual env
				if (selection === 'Yes') {
					fixVirtualEnv();
			...
			vscode.window.showInformationMessage('[IMPORTANT: Mac users MUST install python3.9 (must be version 3.9), NodeJS and restart computer before proceeding.] To install dependencies so the extension works properly, allow installation of virtual environment', ...['Yes', 'No'])
			.then(async selection => {
				// installing virtual env
				if (selection === 'Yes') {
					fixVirtualEnv();
